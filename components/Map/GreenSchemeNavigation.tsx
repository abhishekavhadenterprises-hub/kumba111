...
export default function GreenSchemeNavigation() {
  ...
  // Check if Green Scheme is active in the KML folders
  const isGreenSchemeActive = activeKmlFolders.some(id => typeof id === 'string' && id.includes("Green Scheme - For Non Parvani Days"));

  const popupRef = useRef<L.Popup | null>(null);
  const currentRouteName = selectedFeature?.properties?.name;
  const currentRouteIndex = currentRouteName ? GREEN_SCHEME_ROUTES.indexOf(currentRouteName) : -1;
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      Object.values(GREEN_AUDIO_MAP).forEach(url => {
    ...
    const setupVehicleListener = (map: L.Map) => {
      const handleVehicle = (e: Event) => {
        ...
        for (const wp of GREEN_WAYPOINTS) {
      ...
      (window as any)._cleanupGreenVehicleListener = () => window.removeEventListener("vehicle-position", handleVehicle);
    };

    return () => {
      clearInterval(intervalId);
      if ((window as any)._cleanupGreenVehicleListener) {
        (window as any)._cleanupGreenVehicleListener();
        delete (window as any)._cleanupGreenVehicleListener;
      }
    };
  }, []);
  ...
  const handlePrev = () => {
    ...
    if (currentRouteIndex === -1) prevIndex = 0; // default to first if none selected
    navigateTo(prevIndex);
  };

  const handleNext = () => {
    let nextIndex = currentRouteIndex < GREEN_SCHEME_ROUTES.length - 1 ? currentRouteIndex + 1 : currentRouteIndex;
    if (currentRouteIndex === -1) nextIndex = 0; // default to first if none selected
    navigateTo(nextIndex);
  };

  if (!isGreenSchemeActive) return null;
  ...
  const hasNext = currentRouteIndex === -1 || currentRouteIndex < GREEN_SCHEME_ROUTES.length - 1;

  // Determine the names of the next/prev routes
  const prevRouteName = hasPrev ? GREEN_SCHEME_ROUTES[currentRouteIndex - 1] : "";
  let nextIndex = 0;
  if (currentRouteIndex !== -1 && currentRouteIndex < GREEN_SCHEME_ROUTES.length - 1) {
    nextIndex = currentRouteIndex + 1;
  }
  const nextRouteName = hasNext ? GREEN_SCHEME_ROUTES[nextIndex] : "";

  // Helper to format route name cleanly (remove trailing "_red" or brackets if any, though Green doesn't have them, it's good practice)
    ...
    <>
      ...
      {hasPrev && (
        <div className="absolute top-1/2 -translate-y-1/2 z-[400] pointer-events-auto group transition-all duration-300" style={{ left: "calc(var(--left-sidebar-width, 0px) + 16px)" }}>
          <button
            onClick={handlePrev}
            className="flex items-center gap-1.5 px-3 py-2.5 bg-white/95 backdrop-blur-md border border-emerald-200 rounded-lg shadow-[0_0_15px_rgba(16,185,129,0.15)] text-emerald-700 hover:bg-emerald-50 transition-all"
          >
          ...
          {/* Hover Detail */}
          <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 pointer-events-none opacity-0 group-hover:opacity-100 transition-all duration-300 w-48 bg-white/95 backdrop-blur-md border border-emerald-200 p-2.5 rounded-xl shadow-[0_8px_30px_rgba(16,185,129,0.15)] translate-x-2 group-hover:translate-x-0">
            <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-wider block mb-1">Previous Route</span>
      ...
      {hasNext && (
        <div className="absolute top-1/2 -translate-y-1/2 z-[400] pointer-events-auto group transition-all duration-300" style={{ right: "calc(var(--right-sidebar-width, 0px) + 16px)" }}>
          ...
          {/* Hover Detail */}
          <div className="absolute right-full top-1/2 -translate-y-1/2 mr-3 pointer-events-none opacity-0 group-hover:opacity-100 transition-all duration-300 w-48 bg-white/95 backdrop-blur-md border border-emerald-200 p-2.5 rounded-xl shadow-[0_8px_30px_rgba(16,185,129,0.15)] -translate-x-2 group-hover:translate-x-0">
            <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-wider block mb-1 text-right">Next Route</span>
